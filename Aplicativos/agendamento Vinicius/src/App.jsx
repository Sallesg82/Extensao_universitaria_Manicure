  ...
  try {
    ...
    if (searchInput && !searchInput._wired) {
      ...
      searchInput.addEventListener('input', function() {
        const regularSection = document.getElementById('regular-clients-section')
        if (regularSection && regularSection.style.display !== 'none') return
        const activeTab = document.querySelector('.filter-tab.active')
        const activeFilter = (activeTab && activeTab.dataset.filter) || 'all'
        const q = this.value.trim().toLowerCase()
        let base = activeFilter === 'all' ? clientsCache : clientsCache.filter(c => c.status === activeFilter)
        if (q) {
          base = base.filter(c => c.name.toLowerCase().includes(q) || (c.phone || '').includes(q))
        }
        renderClientList(base)
      })
    }
  } catch (e) {
...
function filterClients(filter, tabEl) {
  ...
  if (filter === 'regulares') {
    ...
    if (clientsLayout) clientsLayout.style.display = 'none'
    renderRegularClients()
    return
  }

  if (regularSection) regularSection.style.display = 'none'
  if (clientsLayout) clientsLayout.style.display = ''

  const searchVal = (document.getElementById('client-search') || {}).value || ''
  let base = filter === 'all' ? clientsCache : clientsCache.filter(c => c.status === filter)
  if (searchVal.trim()) {
...
function renderRegularClients() {
  const sorted = [...clientsCache].sort((a, b) => (b.visits || 0) - (a.visits || 0))
  const minVisits = sorted.length > 0 ? Math.max(2, Math.floor(sorted[0].visits * 0.3)) : 2
  const regulars = sorted.filter(c => (c.visits || 0) >= minVisits).slice(0, 20)

  const countEl = document.getElementById('regular-clients-count')
  if (countEl) countEl.textContent = regulars.length + ' clientes'

  const list = document.getElementById('regular-client-list')
  if (!list) return

  if (regulars.length === 0) {
    list.innerHTML = '<div style="padding:20px;text-align:center;opacity:0.5;">Nenhum cliente regular encontrado.</div>'
    return
  }

  list.innerHTML = regulars.map((c, i) => {
    const lastDate = c.last_visit ? c.last_visit.split('T')[0].split('-').reverse().join('/') : '—'
    const statusMap = { frequente: 'Frequente', regular: 'Regular', novo: 'Novo', inativo: 'Inativo' }
    const statusClassMap = { frequente: 'status-done', regular: 'status-confirmed', novo: 'status-pending', inativo: 'status-pending' }
    const label = statusMap[c.status] || c.status
    ...
    return `
      <div class="client-row" onclick="selectClient(this, ${c.id})">
        <div class="client-info-cell">
          <span style="font-size:1.1em;margin-right:6px;min-width:22px;display:inline-block;">${medal}</span>
          <div class="client-av" style="background:${c.avatar_bg};color:${c.avatar_color};">${c.avatar_initials}</div>
          <div>
            <div class="client-name-cell">${c.name}</div>
            <div class="client-phone">${c.phone}</div>
        ...
        <div class="td" style="font-weight:700;color:var(--accent);">${c.visits} visitas</div>
        <div class="td">${lastDate}</div>
        <div class="td money">${spent}</div>
        <span class="appt-status ${cls}">${label}</span>
      </div>`
